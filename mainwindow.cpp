...
#ifdef SMXX_TEST
	leftBlockSMXXutton = new QPushButton(leftSwitchBlockWidget);
	leftBlockSMXXutton->setObjectName("blockBCMButton");
	leftBlockSMXXutton->setText("SMxx");
	leftSwitchBlockVLayout->addWidget(leftBlockSMXXutton);

	connect(leftBlockSMXXutton, &QPushButton::clicked, this, &MainWindow::slot_leftBlockSMXXutton_clicked);
#else
...
void MainWindow::initConfig()
{
	...
	{
#ifdef SMXX_TEST
		leftBlockSMXXutton->click();

		QString version = list[2].remove(list[2].left(4));
		...
		{
			//leftBlockDMButton->blockSignals(true);
			leftBlockDMButton->click();
			//leftBlockDMButton->blockSignals(false);
		}
		else
		{
			//leftBlockBCMButton->blockSignals(true);
			leftBlockBCMButton->click();
			//leftBlockBCMButton->blockSignals(false);
...
void MainWindow::resetConfig()
{
	...
	else if (viewWindowState->selectedBlock == TestBlockName::SMXX)
		data[1] += "SMXX" + selectBlockVersionComboBox->currentText() + ";";
...
#ifdef SMXX_TEST
		leftBlockSMXXutton->setStyleSheet(lightStyles.alwaysActiveStandButton);
#else
...
#ifdef SMXX_TEST
		leftBlockSMXXutton->setStyleSheet(darkStyles.alwaysActiveStandButton);
#else
		if (viewWindowState->selectedBlock == TestBlockName::BCM) // button
		{
...
#ifdef SMXX_TEST
		leftBlockSMXXutton->setToolTip(QString("Проверка блока SMXX"));
#else
...
#ifdef SMXX_TEST
		leftBlockSMXXutton->setToolTip(QString("Checking the SMXX"));
#else
...
void MainWindow::slot_verificationtestTestManualStandButton_clicked()
{
	...
	//connect(can, &Can::Signal_ChangedStatusBlockConnect, dlgVerification, &VerificationTest::Slot_ChangedStatusStandConnect);
...
void MainWindow::createTestWindow(WindowType testType, std::vector<Cable> preparedCables)
{
	...
	connect(can, &Can::Signal_ChangedStatusBlockConnect, testWindow, &TestWindow::Slot_ChangedStatusBlockConnect);
	...
	loadCables(viewWindowState->selectedBlock, viewWindowState->actualVersion);
	resetWindowView();
	can->deinitCan();
...
static Cable fillCable(int id, ConnectorId connector, int pin, int direction, int type, int canId, int bit, std::vector<Thresholds> thresholdsManual, std::vector<Thresholds> thresholdsAuto, QString name, QString component)
{
	Cable cable;
	cable.setId(id);
	...
	cable.setBit(bit);
	cable.setThresholdsManual(thresholdsManual);
	cable.setThresholdsAuto(thresholdsAuto);
	cable.setName(name);
	...
	for (int i = 0; i < data.size(); i++)
	{
		...
		if (line[0] == ":")
		{
			...
			case TestBlockName::SMXX:
				blockVersionsSMXX.push_back(line);
				break;
...
void MainWindow::loadCables(TestBlockName block, QString version)
{
	...
	for(int i = 0; i < data.size();i++)
	{
		...
		if (admissionVersion)
		{
			...
			id += 1;
			if (list.size() == 6) // information bus
			{
				Cable cable;
				cable.setId(id);
			...
			else if (list[0] == 'S') // sleep mode
			{


				for (int j = 1; j < list.size(); j += 4)
				{
					Cable cable;
					std::vector<Thresholds> thresholdsAuto;
					cable.setConnector((ConnectorId)list[j].toInt());
					if(list[j + 1].contains('|'))
						cable.setComponent(list[j + 1]);
					else
						cable.setPin(list[j + 1].toInt());
					cable.setName("Sleep");
					thresholdsAuto.push_back(Thresholds(-1, -1, list[j + 2].split(u'|')[0].toDouble(), list[j + 2].split(u'|')[1].toDouble()));
					//thresholdsAuto.push_back(Thresholds(-1, -1, list[j + 3].split(u'|')[0].toDouble(), list[j + 3].split(u'|')[1].toDouble()));
					cable.setThresholdsAuto(thresholdsAuto);
					cables.push_back(cable);
				}
			...
			for (int j = 8; j < list.size(); j += 1)
			{
				QStringList listThresholds = list[j].split(u'|');
				if (listThresholds[0] == 'M')
				{
					int minValue = listThresholds[1].toInt();
					int maxValue = listThresholds[2].toInt();
					thresholdsManual.push_back(Thresholds(minValue, maxValue));
				}
				else if(listThresholds[0] == 'A')
					if (direction == DIRECTION_OUT)
					{
						double minCurrent = listThresholds[1].toDouble();
						double maxCurrent = listThresholds[2].toDouble();
						double minVoltage = listThresholds[3].toDouble();
						double maxVoltage = listThresholds[4].toDouble();
						int minValue =		listThresholds[5].toInt();
						int maxValue =		listThresholds[6].toInt();

						thresholdsAuto.push_back(Thresholds(minCurrent, maxCurrent, minVoltage, maxVoltage, minValue, maxValue));
					...
					else
					{
						int minValue = listThresholds[1].toInt();
						int maxValue = listThresholds[2].toInt();
						thresholdsAuto.push_back(Thresholds(minValue, maxValue));


					}
			}
			cables.push_back(fillCable(id, connector, pin, direction, type, canId, bit, thresholdsManual, thresholdsAuto, name, component));
		}
	}
}
...
void MainWindow::slot_leftBlockBCMButton_clicked()
{
	...
	{
		selectBlockVersionComboBox->blockSignals(true);
		...
		if (isAllInit)
		{
			loadCables(viewWindowState->selectedBlock, selectBlockVersionComboBox->itemText((int)blockVersionsBCM.size() - 1));
			resetConfig();
		}
		selectBlockVersionComboBox->blockSignals(false);
	}
}
...
void MainWindow::slot_leftBlockDMButton_clicked()
{
	if (viewWindowState->selectedBlock != TestBlockName::DTM)
	{
		...
		if (isAllInit)
		{
			...
			loadCables(viewWindowState->selectedBlock, selectBlockVersionComboBox->itemText((int)blockVersionsDTM.size() - 1));
			resetConfig();
		}
		selectBlockVersionComboBox->blockSignals(false);
	}
...
void MainWindow::slot_leftBlockSMXXutton_clicked()
{
	if (viewWindowState->selectedBlock != TestBlockName::SMXX)
	{
		selectBlockVersionComboBox->blockSignals(true);
		viewWindowState->selectedBlock = TestBlockName::SMXX;
		resetTheme();

		selectBlockVersionComboBox->clear();
		for (int i = 0; i < blockVersionsSMXX.size(); i++)
			selectBlockVersionComboBox->addItem(blockVersionsSMXX[i]);
		selectBlockVersionComboBox->setCurrentIndex((int)blockVersionsSMXX.size() - 1);
		switchStyleMainButtons();
		if (isAllInit)
		{
			loadCables(viewWindowState->selectedBlock, selectBlockVersionComboBox->itemText((int)blockVersionsSMXX.size() - 1));
			resetConfig();
		}
		selectBlockVersionComboBox->blockSignals(false);
	}
...
bool MainWindow::GenerateNewArchiv(QStringList filesToAdd)
{
