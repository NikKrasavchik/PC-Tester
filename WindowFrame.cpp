...
void WindowFrame::setTitle(WindowType windowType) {
	...
	case RUSSIAN_LANG:
		...
		case WindowType::CONFIGURATOR:
			ui->title->setText(QString("Конфигуратор") + tmpStr);
			break;

		case WindowType::IN_TEST_MANUAL_STAND:
			ui->title->setText(QString("Входы | Ручной стенд") + tmpStr);
			break;

		case WindowType::OUT_TEST_MANUAL_STAND:
			ui->title->setText(QString("Выходы | Ручной стенд") + tmpStr);
			break;

		case WindowType::FULL_TEST_MANUAL_STAND:
			ui->title->setText(QString("Полная проверка | Ручной стенд") + tmpStr);
			break;

		case WindowType::IN_MANUAL_TEST_AUTO_STAND:
			ui->title->setText(QString("Входы ручная проверка | Автоматический стенд") + tmpStr);
			break;

		case WindowType::OUT_MANUAL_TEST_AUTO_STAND:
			ui->title->setText(QString("Выходы ручная проверка | Автоматический стенд") + tmpStr);
			break;

		case WindowType::IN_AUTO_TEST_AUTO_STAND:
			ui->title->setText(QString("Входы автоматическая проверка | Автьоматический стенд") + tmpStr);
			break;

		case WindowType::OUT_AUTO_TEST_AUTO_STAND:
			ui->title->setText(QString("Выходы автоматическая проверка | Автоматический стенд") + tmpStr);
			break;

		case WindowType::FULL_TEST_AUTO_STAND:
			ui->title->setText(QString("Полная автгоматическая проверка | Автоматический стенд") + tmpStr);
			break;

		case WindowType::MOREWINDOW:
			ui->title->setText(QString("Подробнее...") + tmpStr);
			break;
